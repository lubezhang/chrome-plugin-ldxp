...
- 在目标小铺页按价格升序、价格降序或网站默认顺序排列商品，并可隐藏缺货商品。
- 在“设置”页保存订单联系方式；安全密码默认仅在当前浏览器会话中保留，也可选择长期记住。
- 在“订单”页完成人机验证后查询订单，支持按商品名或订单号搜索、按订单状态筛选及分页浏览。
- 卡密默认遮罩显示，支持单条显示、隐藏和复制，以及一键显示或复制当前订单的全部卡密。
...
3. 打开“订单”页，按提示完成人机验证后搜索、筛选订单或查看卡密。
4. 提交订单前，请自行核对订单确认弹窗中的联系方式和安全密码。

## 本地存储与隐私

扩展使用 Chrome 的本地扩展存储保存商品排序、缺货过滤、联系方式及“长期记住密码”设置。未选择长期记住时，安全密码和订单查询验证票据只保存在 Chrome 会话存储中，并在退出浏览器后清除。

联系方式和安全密码不会被写入扩展安装包，也不会发送给第三方服务。它们仅在用户主动查询订单、读取卡密或确认下单时，按 WZYP 页面和接口要求使用。选择长期记住后，安全密码将以浏览器扩展本地存储的明文形式保存，请仅在受信任的浏览器配置中启用。
...
src/background.js 管理仅在当前浏览器会话内有效的敏感信息
